...
%\ref{fig:12_gr}.
%\begin{itemize}
    %\item [\boldmath$G_1\,$] A simple deterministic network consisting of
        %a triangle of center-nodes each connected to hexagons.
    %\item [\boldmath$G_2\,$] A more complex deterministic network consisting
        %of 9 center-nodes each connected to 20-gons.
    %\item [\boldmath$G_3\,$] A random network with a grouping structure of
        %$4\times 25$ nodes. Each node is connected to nodes in its own
        %group with $p\ix{g}=0.75$ and to nodes in other groups with $p\ix{o}
        %= 0.1$.
%\end{itemize}
...
%to be smaller. With $p\ix{g} = 0.65$ and $p\ix{o} = 0.15$ for example, the desired
%decomposition into four rings was not achieved.

%\subsection{From Random Walks to Communities}
%As the last task of this section, the stability curves $t\mapsto r(H, t)$
%of the four networks $G_0$, $G_1$, $G_2$, and $G_3$ for different
%partitionings $H$ (obtained in the same way as above) were calculated. The
%resulting stability curves for time scales $1\le t \le 100$ can be seen in
%figure \ref{fig:13}.

%The results are as expected and show that the stability is often
%best for the same partitionings $H$ that also exhibit the highest
%modularity and that the community structures found before are indeed
%relatively stable compared to other structures. For large time scales
%though, one can see for $G_2$ and $G_3$ that partitionings with fewer
%communities than the expected value $h = 9$ and $h=4$ are more stable.
%Since longer random walks cover longer distances on average, this behavior
%is not surprising. 
...
%\begin{figure}
    ...
    %Here $h$ denotes the number of communities in the respective visualized
...
%perspective (e.g by considering degree histograms, average shortest path
%lengths, etc.), but for real network applications much information can
%likely be found in more local structures. Therefore, we introduce the
...
%One possible criterion for relevance is the comparison of occurrence in the
...
%percentage of $k$-node subgraphs $S_k$ of $G$ that are isomorphic to $M$,
%i.e. where there exists a permutation of its nodes so that it is identical
%to $M$. Let $\chi_M$ be the characteristic function of $M$ in $S_k$, that is
%$\chi_M(g) = 1$ iff $g\in S_k$ is isomorphic to $M$. 
...
%we sample this sum (Monte-Carlo technique) with the probability $p(g)$ of
%choosing the sample $g$ as
%\begin{align}
...
%\end{align} 
%where $r_{ij}$ denotes the geographical distance between the points $i$ and $j$, and $A_{ij}$ is the adjacency matrix.
%The path length between two connected points is 
%\begin{align}
    %l_{ij} = (1-\delta) \sqrt{N} r_{ij} + \delta.
%\end{align}
 
%Finding the exact network for which $K$ is minimized is next to impossible,
...
%An algorithms specifically designed to tackle these kind of problems is
...
%would be higher. The acceptance probability of worse states is inspired by thermodynamics
...
%$\alpha=0.1$ and $\delta \in \{0, 0.5, 1\}$ results in the optimized
%networks shown in figure \ref{fig:motifs}. Here $T$ was chosen to be 0.01
%initially and was decreased by factor 0.99 in each of the 1000 cooling
%steps (this parameter, however, is not relevant for the model, but affects
%the performance of the annealing and thus the quality of the solution
%found). For $\delta=0$, the results resemble a road network, as switching
%roads is essentially free, whereas $\delta=1$ looks like airplane routes,
%because waiting times at airports can constitute a significant amount of
%the travel time, so one central node connected to many others becomes more
%attractive to the algorithm.  The result is not a perfect star topology,
%because the algorithm only finds an approximative solution.
...
%\begin{minipage}[b]{.45\linewidth}
  ...
  %\end{minipage}
	...
	%\end{minipage}
    %\caption{Network structure for $25$ random points in the unit square,
        %optimized for $\delta \in \{0, 0.5, 1\}$, and for a network where
    %the node positions were taken as the locations of the 40 busiest
%European airports for $\delta = 0.5$. }
	%\label{fig:motifs}
%\end{figure}
...
%For this section the interest lies on networks with nodes that are
%understood as persons, so that the edges represent social interactions. In
%this situation, a \emph{social distance} between two persons can be
%implemented by comparing how \enquote{near} these persons are in different
%social categories (Are they in the same sports clubs or classes? Do they
%have similar hobbies and music preferences?). 

%One application of the social distance is a \emph{message passing system}
%that works by iteratively relaying a message to persons that have similar
%social characteristics to the target person.

%\subsection{Creating Artificial Social Networks}
%To create artificial social networks, the algorithm presented in the
%corresponding seminar talk (and described on the tasks sheet for C3) was
%used. This algorithm relies on the following parameters in order to
%construct (1) a social graph $G$ and (2) a (suitable) measure $\rho$ for
%social distances:
%\begin{itemize}
    %\item[\boldmath$S\,$] the number of distinct social characteristics looked at.
    %\item[\boldmath$l\,$] the bits of information to describe the position
        %of a node in one of the social characteristics; so there are $2^l$
        %different social coordinates for every characteristic.
    %\item[\boldmath$k\,$] the average degree of the graph.
    %\item[\boldmath$g\,$] the group size; the number of nodes that have the same
        %social coordinates for one of the characteristics.
    %\item[\boldmath$\alpha\,$] an inverse temperature-like parameter that determines how strong the
        %group structures are. Low values of $\alpha$ make the groups open
        %and permeable to other groups, while high $\alpha$ result
        %in a tighter isolation of the single groups.
%\end{itemize}
%In the following, networks resulting for $l = 3, S\in\{1, 2\}, g = 5, k = 4$, and
%$\alpha\in\{1, 5\}$ are analyzed. Examples for obtained networks (and their
%decompositions, which are important for the next subsection) can be found in
%figures \ref{fig:331} ($S = 1$) and \ref{fig:332} ($S = 2$), labeled by the term
%\enquote{original}.

%Networks created for $S = 1$ and $\alpha = 1$ were most times connected,
%and the drawing routines of \texttt{networkx} composed the layout in such
%a way, that nodes of the same groups were put near to each other in trend.
%Though a large part of the inter-group edges connected two groups of social
%distance $\rho = 1$, edges overcoming $\rho = 2$ and even $\rho = 3$ (though
%quite seldom) were also observed.

%For $S = 1$ and $\alpha = 5$, on the other hand, no created instance was
%connected and in virtually all cases, inter-group edges only formed between
%groups with distance $\rho = 1$. Though one could naively expect that there
%should be many instances, where all groups are already perfectly separated
%for $k = 4$ and this large an $\alpha$, this was not observed once when
%generating different trial networks. The reason is that $k$ only is the
%average degree, so it is quite likely that at least one node already having
%four links to all of its group members is chosen to create another connection --
%this must then be an inter-group edge.

%For $S = 2$ the observable interactions are more complex. Looking at
%\ref{fig:332} one can see that \texttt{networkx} places both the colors
%and the numbers (representing the two different social characteristics),
%in a (more or less) localized fashion, so that there are areas where one
%is likely to find the same color or number several times. This effect,
%of course, is stronger for $\alpha = 5$ than for $\alpha = 1$.

%\subsection{Decomposing Artificial Social Networks}
%After having applied this algorithm to create social networks, the next
%question was, how strong the group structure is actually ingrained in the
%created graphs itself. Put in other words: Can one reconstruct the groups
%when only knowing about the bare graph? And if one can't do it exactly, how
%close does one get?

%To decompose the graphs in communities, the algorithm introduced in C1,
%based on the shortest-path betweenness centrality, was used.
%The decompositions obtained for different numbers $h$ of communities are
%(again) given in figure \ref{fig:331} for $S = 1$ and figure \ref{fig:332}
%for $S = 2$.

%For $S = 1$ and $\alpha = 1$, the resulting partitionings exhibit some tendency that
%similar colors are indeed more likely be found in the same subgraphs (see
%e.g.\ the two-node subgraphs for $h = 12$). Nevertheless, the used method
%seems to be unsuitable to get reliable information about the social
%structure.
%The communities obtained for $S=1, \alpha = 5$ meanwhile respect the
%original groups very consistently over many trials, so the algorithm seems
%to be appropriate for guessing the groups structure in this case.

%For $S=2$ and $\alpha = 1$ the decompositions show some slight traces of
%the original groups: one likely has a greater chance of having the same
%number and/or color in the same subgraph than it would be for a random
%constellation. For $\alpha = 5$ this trend is much stronger; one can find
%several subgraphs that are of the same number or color. As it is
%impossible to find an exact decomposition respecting the group structure
%for two different social characteristics, the algorithm seems to do a good
%job for high $\alpha$, often extracting at least one of the two
%characteristics.


%\subsection{Message Passing}
%The main task of this section is to implemented \emph{message passing}
%meaning that a message from node $i$ to node $j$ is iteratively relayed to
%neighbouring nodes that have the least social distance $\rho$ to $j$. 
%In the following, messaging between randomly chosen nodes $i$ and $j$ with a
%fixed value $d$ for the shortest-path distance is analyzed. It is clear
%that the pathway of the massage will have a length $\lambda \ge d$.

%Figure \ref{fig:333} shows the calculated frequency distribution of
%$\lambda$ when sending messages between nodes with $d\in\{1, 2, 3, 4\}$ for
%the social graphs described above. It is noticeable that the distributions
%become more uniformly for higher $d$, which indicates that this way of
%transmitting messages gets less efficient for higher $d$. The strongly
%increasing failure rate (meaning that the massage did not make it in time
%-- which is after 25 steps) reinforces this observation.

%\subsection{Hub Nodes for Message Passing}
%Looking at the relay distribution for the message passing processes
%(not included in the protocol) reveals that some nodes indeed conduct
%a very high number of relays when compared to other nodes.
%The exact reason why these nodes are this important for the passing of messages
%probably depends on several aspects. Suitable factors to assess the
%importance of nodes for massage passing could e.g.\ be the following: 
%\begin{itemize}
    %\item The \emph{degree} of the corresponding node. Nodes with a higher degree
       %are of course more likely to get chosen in the passing algorithm.
   %\item The \emph{betweenness-centrality} value of the node. Nodes with
       %a high betweenness tend to communicate between different groups,
       %which is what is needed for efficient message passing.
   %\item The \emph{diversity} of a node. Diversity here means, that the
       %respective node is part of many different groups by exhibiting
       %a constellation of social coordinates rather unlikely for the rest
       %of his peers. These nodes can function as \enquote{entry point}, if
       %outsiders want to approach the group (thus the diversity is connected
       %to the betweenness, but it may use the social distance as additional
       %information).
%\end{itemize}
%When doing some scatter plots of (1) the nodes' degrees and (2) the
%betweenness against the number of relays, some (rather slight) correlations
%were found that indeed suggest that the criterions (1) and (2) are at least
%somehow necessary conditions for being a hub node for message passing (meaning
%that nodes with very low degree or very low betweenness also have low relay
%numbers, but that high degrees and high betweenness values are not
%sufficient for a high number of relays).


%\begin{figure}
    %\centering
    %\def\svgwidth{0.9\textwidth}
    %\input{311_new.pdf_tex}
    %\caption{The generated social graphs for $S=1$ and \textbf{(a)} $\alpha = 1$
    %as well as \textbf{(b)} $\alpha = 5$. The other parameters were set to
    %$g = 5, l = 3$, and $k = 4$. The colors represent the different
    %social groups that are given by the nodes social coordinates. The two
    %topmost graphs (\enquote{original}) are the full networks containing
    %all edges, whereas the middle ($h = 4$) and lowermost ($h=8$) graphs
    %represent the community structure derived by the decomposition
    %algorithm of C1 when dividing the graph in
    %$h = 8$ respectively $h = 12$ communities. Visualizing the decomposed
    %graphs was done by letting \texttt{networkx} plot the respective
    %community-subgraphs of the decomposition.}
    %\label{fig:331}
%\end{figure}
...
%\begin{figure}
    ...
    %7).  Similar to figure \ref{fig:331}, the two upper graphs correspond to
    %the full networks, while the other graphs correspond to decompositions
    %in $h = 12$ communities, using the algorithm introduced in C1.}
    %\label{fig:332}
%\end{figure}

%\begin{figure}[bcht]
    %\centering
    %\gnuplotloadfile[terminal=epslatex, terminaloptions={color size 6.5,6.0}]{pictures/33.gp}
    %\caption{Relative frequencies of the path lengths $\lambda$ that occur when
        %socially transferring messages between nodes with path-distance $d\in\{1,2,3,4\}$.
        %When more than 25 steps were necessary,
        %the messaging process failed. To obtain the data, $200$ social
        %graphs with parameters $g = 5, l = 3, k = 4, s = 2, \alpha = 1$
        %were created and were then used to conduct $250$ message transfers
        %each for all values of $d$.}
    %\label{fig:333}
%\end{figure}
