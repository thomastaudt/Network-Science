...
\begin{figure}[h]
    ...
    \caption{Relative power transferred by the connection
        ...
        whereas the transmission behaves eratically for (c). }
    \label{fig:B31}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Relative size of the largest mutually connected cluster of two interdependend Erdös-Rényi graphs, for different initial mean degrees $\left< k \right>$ and network sizes $N$, plotted against $p \left< k \right>$, where $1-p$ is the probability of failure. We see that there is a critical value $p_c \left< k \right> \approx 2.2$ where the network percolates that is independent of $N$ and $\left< k \right>.}
    \label{fig:B32}
\end{figure}
\end{document}
